...
function filterProductsByCategory(category: string, products: any[]) { let filteredProducts = [];
    ...
    document.addEventListener("DOMContentLoaded", function() { // Attach click event to category links const categoryLinks = document.querySelectorAll(".category-link"); categoryLinks.forEach(link => { link.addEventListener('click', function(event) { event.preventDefault(); const categoryName = this.getAttribute('data-category');
    ...
    function displayProducts(products: any[]) { const productsContainer = document.getElementById('category-products'); productsContainer!.innerHTML = '';
